...
def compare_dbs_full():
    ...
    def get_tables(server, db, user, pwd):
        ...
        try:
            ...
            cursor = conn.cursor()
            # Select ALL base tables without filtering by prefix
    ...
    print(f"\nALL TABLES IN DEV BUT MISSING IN PRODUCTION ({prod_name}):")
    for t in missing_in_prod:
        # Ignore only system tables (like sysdiagrams) if they appear
        if t.lower() != 'sysdiagrams':
            print(f"  - {t}")
