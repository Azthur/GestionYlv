...
async function loadUserVendedores() {
    ...
    try {
        ...
        html += `<div style="grid-column: 1 / -1; margin-bottom: 0.5rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border);"><h4 style="color:var(--primary); margin:0;">Lista de Vendedores Disponibles</h4><small style="color:var(--text-muted);">Seleccione los vendedores que este usuario podrá visualizar.</small></div>`;
        html += allVendors.map(v => {
            const cod = (v.codigo || '').trim();
            const nom = (v.nombre || '').trim();
            const checked = assignedSet.has(cod) ? 'checked' : '';
            return `<label style="display:flex;align-items:center;gap:0.5rem;padding:0.6rem 1rem;border-radius:8px;border:1px solid var(--border);cursor:pointer;transition:all 0.2s;font-size:0.85rem; background: #f8fafc;">
...
async function loadUserTiendas() {
    ...
    try {
        ...
        html += `<div style="grid-column: 1 / -1; margin-bottom: 0.5rem; padding-bottom: 0.5rem; border-bottom: 1px solid var(--border);"><h4 style="color:#8b5cf6; margin:0;">Lista de Tiendas Rendición Disponibles</h4><small style="color:var(--text-muted);">Seleccione las tiendas que este usuario podrá visualizar (Aplica si no tiene vendedores asignados).</small></div>`;
        html += allItems.map(v => {
            const cod = (v.codigo || '').trim();
            const nom = (v.nombre || '').trim();
            const checked = assignedSet.has(cod) ? 'checked' : '';
            return `<label style="display:flex;align-items:center;gap:0.5rem;padding:0.6rem 1rem;border-radius:8px;border:1px solid rgba(139, 92, 246, 0.3);cursor:pointer;transition:all 0.2s;font-size:0.85rem; background: #f5f3ff;">
                <input type="checkbox" class="tienda-cb" value="${cod}" ${checked} style="width:16px;height:16px;cursor:pointer;">
                <strong style="color:#6d28d9;">${cod}</strong> - ${nom}
            </label>`;
        }).join('');
