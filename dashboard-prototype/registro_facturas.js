    ...
    try {
        ...
        if (c.FacturaId && c.NroOrdenCompra) {
            const result = await Swal.fire({
                ...
                html: `Esta factura ya se encuentra vinculada a la OC N° <strong>${c.NroOrdenCompra.split(',').map(oc => (c.TipoOc||'') + oc.trim()).join(', ')}</strong> con estado <strong>${c.FacturaEstado||'Registrada'}</strong>.<br>¿Desea continuar?`,
...
async function loadOCDetails(nrodoc, tipooc, anos, ruc, prov, moneda, factVincStr) {
    ...
    try {
        ...
        // El input invNroOC ya fue actualizado al inicio de la función
