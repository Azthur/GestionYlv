...
def get_ocs_disponibles(
    ...
    \"\"\"
    Obtener OCs filtradas por año/mes, aplicando dependencias de flujo según el área y validación de usuarios Logísticos.
    \"\"\"
    ...
    try:
        ...
        if tipo_cargo == 'LOG_A_CONT' and login_clean:
            ...
            # Para contabilidad aplica a todos sin reglas de permisos especiales de logistica,
            # pero SI se respeta 'is_only_mine' y 'tipo_oc'
            puede_ver_todo = True
            allowed_types = ['M', 'S', 'T']

        # ═══════════════════════════════════════════════════════════
        # PASO 1: Query ULTRALIGERA — solo filtrar OCs, cero JOINs
        # ═══════════════════════════════════════════════════════════
        query = \"\"\"
            SELECT 
        ...
        if login_clean:
            ...
            if tipo_oc != 'ALL':
                ...
                if not puede_ver_todo and allowed_types:
                    ...
                    query += f" AND o.TipoOc IN ({placeholders})"
        ...
        # ═══════════════════════════════════════════════════════════
        # PASO 2: Enriquecer en CHUNKS de 50 (evita IN(...) enormes)
        # ═══════════════════════════════════════════════════════════
        nrodocs = list(set(r['nrodoc'] for r in ocs_raw))
        CHUNK = 1000  # SQL Server rinde mejor con IN() de máximo 50 items
        ...
        for chunk in chunked(nrodocs, CHUNK):
            ...
            try:
                cursor.execute(f\"\"\"
                    SELECT RTRIM(NroOrdenCompra), RTRIM(Serie) + '-' + RTRIM(Numero),
                           Total, FecEmision, Uuid, Id
                    FROM CntFacturaCab 
                    WHERE NroOrdenCompra IN ({ph}) AND Estado != 'Anulada'
                \"\"\", tuple(chunk))
                for row in cursor.fetchall():
                    factura_map[row[0].strip()] = {
                        'factura': row[1], 'total_factura': row[2],
                        'fec_factura': row[3], 'factura_uuid': row[4], 'fac_id': row[5]
                    }
            except Exception as ex:
                pass

        # 2b. Cant Pedida (CmpROcom)
        pedida_map = {}
        for chunk in chunked(nrodocs, CHUNK):
            ph = ",".join(["?"] * len(chunk))
            try:
        ...
        # 2d. Observaciones de rechazo
        rechazo_map = {}
        for chunk in chunked(nrodocs, CHUNK):
            ph = ",".join(["?"] * len(chunk))
            try:
                cursor.execute(f\"\"\"
                    SELECT RTRIM(d.NroOrdenCompra), RTRIM(d.ObservacionRechazo)
                    FROM CntCargosDetalle d
                    INNER JOIN CntCargosDocumentales c ON d.CargoId = c.Id
                    WHERE d.NroOrdenCompra IN ({ph})
                      AND c.CodCia = ? AND d.EstadoContable = 'RECHAZADO' AND c.TipoCargo = ?
                \"\"\", tuple(chunk) + (codcia, tipo_cargo))
                for row in cursor.fetchall():
                    rechazo_map[row[0].strip()] = row[1]
            except:
                pass

        # 2e. Cargos existentes (filtro de flujo)
        ...
        if tipo_cargo == 'LOG_A_CONT':
            for chunk in chunked(nrodocs, CHUNK):
                ...
                try:
                    cursor.execute(f\"\"\"
                        SELECT DISTINCT RTRIM(d.NroOrdenCompra)
                        FROM CntCargosDetalle d
                        INNER JOIN CntCargosDocumentales c ON d.CargoId = c.Id
                        WHERE d.NroOrdenCompra IN ({ph})
                          AND c.CodCia = ? AND c.Estado != 'ANULADO'
            ...
            for chunk in chunked(nrodocs, CHUNK):
                ph = ",".join(["?"] * len(chunk))
                # 1. Traer Ocs que fueron emitidas a logistica en general (para saber si existen o no)
                try:
                    cursor.execute(f\"\"\"
                        SELECT DISTINCT RTRIM(d.NroOrdenCompra)
                        FROM CntCargosDetalle d
                ...
                try:
                    cursor.execute(f\"\"\"
                        ...
                        WHERE d.NroOrdenCompra IN ({ph})
                          AND c.CodCia = ? AND c.Estado != 'ANULADO'
                          AND c.TipoCargo = 'LOG_A_CONT' AND d.EstadoContable = 'ACEPTADO'
                    \"\"\", tuple(chunk) + (codcia,))
                    aceptados.update(row[0].strip() for row in cursor.fetchall())
                except:
                    pass
                
                # 3. Ya enviadas a tesoreria (para evitar duplicidad)
                try:
                    cursor.execute(f\"\"\"
                        SELECT DISTINCT RTRIM(d.NroOrdenCompra)
                        FROM CntCargosDetalle d
        ...
        # ═══════════════════════════════════════════════════════════
        # PASO 3: Ensamblar resultados en Python
        # ═══════════════════════════════════════════════════════════
        results = []
        for d in ocs_raw:
            nro = d['nrodoc']
            ...
            # Filtro de flujo
            if tipo_cargo == 'LOG_A_CONT':
                if nro in cargos_existentes_log:
                    continue  # Ya está en un cargo Logístico activo
            elif tipo_cargo == 'CONT_A_TES':
                if nro in tes_existentes:
                    continue # Ya está en un cargo activo a Tesoreria
                
                # Si existe en Logística PERO NO ha sido aceptada, no se puede mandar
                if nro in cargos_existentes_log and nro not in aceptados:
                    continue
                
                # Si NO existe en logística y "ocs_directas" NO está activo, lo descartamos
                if nro not in aceptados and not is_directas:
                    continue
            ...
            if fac_info:
                ...
                d.update({'factura': None, 'total_factura': 0, 'fec_factura': None, 'factura_uuid': None})

            # Cantidades y Almacén
            ...
            d['status_almacen'] = status_almt
            d['observacion_rechazo'] = rechazo_map.get(nro)
            
            # Formato de fechas 
            if getattr(d['fchdoc'], 'strftime', None):
                d['fchdoc'] = d['fchdoc'].strftime('%Y-%m-%d')
            if getattr(d['fec_factura'], 'strftime', None):
        ...
        print(f"Error en endpoint /ocs-disponibles: {e}")
...
# Extract from @router.get("/ocs-disponibles") up to the end of the function (def get_ocs_disponibles)
# We can use Regex to replace
import re
pattern = re.compile(r'@router\.get\("/ocs-disponibles"\).*?\n    finally:\n        if cursor: cursor\.close\(\)\n        if conn: conn\.close\(\)\n', re.DOTALL)

if pattern.search(backend_code):
    backend_code = pattern.sub(new_func + "\n", backend_code)
    with open('c:/SistemaGestionyelave/backend/cargos_documentales.py', 'w', encoding='utf-8') as f:
        f.write(backend_code)
    print("Backend logic patched.")
else:
    print("Could not find get_ocs_disponibles in cargos_documentales.py")
